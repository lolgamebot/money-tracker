...
// Handle stop recurring (cancels future recurrences, keeps dashboard entries)
...
// Handle delete all records (deletes parent template AND all generated child entries)
...
<body class="bg-[#0a0f1e] min-h-screen text-slate-200">
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-white">Recurring Records</h1>
                        ...
                        <tbody class="divide-y divide-slate-800">
                            <?php foreach ($recurringRecords as $record): ?>
                                ...
                                <tr class="hover:bg-slate-800/40 transition-colors">
                                    ...
                                    <td class="py-3.5 px-4 text-slate-400 whitespace-nowrap"><?= $record["date"] ?></td>
                                    <td class="py-3.5 px-4 text-slate-400 whitespace-nowrap">
                                        <?= $record["recurring_end_date"] ? $record["recurring_end_date"] : '<span class="text-indigo-400 font-medium">Infinite</span>' ?>
                                    </td>
                                    <td class="py-3.5 px-4 text-center">
                                        <span class="bg-slate-800 text-slate-300 text-xs px-2.5 py-1 rounded-md font-semibold border border-slate-700">
                                    ...
                                    <td class="py-3.5 px-4 text-right whitespace-nowrap">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="edit.php?id=<?= $record["id"] ?>" 
                                               class="text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white px-2.5 py-1.5 rounded-lg border border-slate-700 transition-colors">
                                                Edit
                                            </a>
                                            <a href="recurring.php?cancel=<?= $record["id"] ?>"
                                               ...
                                               class="text-xs bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30 px-2.5 py-1.5 rounded-lg transition-colors">
                                                Stop
                                            </a>
                                            <a href="recurring.php?delete_all=<?= $record["id"] ?>"
                                               ...
                                               class="text-xs bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/30 px-2.5 py-1.5 rounded-lg font-medium transition-colors">
                                                Delete All Records
                                            </a>
                                        </div>
                                    </td>
                ...
                <?php foreach ($recurringRecords as $record): ?>
                    ...
                    <div class="bg-[#111827] border border-slate-700 rounded-xl p-4 space-y-3 shadow-md">
                        <div class="flex items-start justify-between gap-2">
                            <div>
                                <h3 class="font-bold text-white text-base">
                                    <?= htmlspecialchars($record["description"] ?: ($record["category_name"] . " " . ucfirst($record["type"]))) ?>
                                </h3>
                                <p class="text-xs text-slate-400 mt-0.5">
                                    Started: <?= $record["date"] ?> • Ends: <?= $record["recurring_end_date"] ?: 'Infinite' ?>
                                </p>
                            </div>
                        ...
                        <div class="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-800">
                            ...
                            <span class="bg-slate-800 text-slate-300 text-xs px-2.5 py-0.5 rounded-full border border-slate-700 capitalize">
                                🔄 <?= ucfirst($record["recurring_interval"]) ?>
                            </span>
                        ...
                        <div class="flex flex-wrap items-center justify-end gap-2 pt-3 border-t border-slate-800/80">
                            <a href="edit.php?id=<?= $record["id"] ?>" class="text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded-lg border border-slate-700">Edit</a>
                            <a href="recurring.php?cancel=<?= $record["id"] ?>" onclick="return confirm('Stop recurring schedule? Future entries will stop generating.')" class="text-xs bg-amber-500/10 text-amber-400 border border-amber-500/30 px-3 py-1.5 rounded-lg">Stop</a>
                            <a href="recurring.php?delete_all=<?= $record["id"] ?>" onclick="return confirm('Delete this recurring expense AND ALL <?= $count ?> generated records?')" class="text-xs bg-rose-500/10 text-rose-400 border border-rose-500/30 px-3 py-1.5 rounded-lg font-medium">Delete All Records</a>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
